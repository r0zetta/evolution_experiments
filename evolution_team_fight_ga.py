...
class game_space:
    ...
    def move_agent(self, index, action, atype):
        ...
        if action == 0: # do nothing
            ...
            hit_enemy = False
            if atype == "green":
                if space_val >= self.starts["purple"]:
                    hit_enemy = True
            else:
    ...
    def get_tile_val(self, tile, atype):
        if tile == 0:
            return 0
        if atype == "green":
            if tile >= self.starts["purple"]:
                return 1
            else:
                return -1
        else:
            if tile >= self.starts["green"] and tile < self.starts["purple"]:
                return 1
            else:
                return -1
    ...
    def get_nearest_enemy_offset(self, xpos, ypos, atype):
        ...
        enemy_positions = []
        if atype == "green":
            enemy_positions = np.argwhere(sp>=self.starts["purple"])
        else:
            enemy_positions = np.argwhere((sp>=self.starts["green"]) & (sp<self.starts["purple"]))
        ind = {}
