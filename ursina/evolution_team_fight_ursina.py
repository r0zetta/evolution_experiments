...
class game_space:
    def __init__(self, width, height, num_walls=0, num_agents=5,
        ...
        self.agent_types = ["green", "purple"]
        self.textures = {"green":"hog", "purple":"yak"}
        self.starts = {"green":2, "purple":2000}
        self.num_agents = {}
        self.num_agents["green"] = num_agents
        self.num_agents["purple"] = num_agents
        self.food_id = 10000
        self.genome_pool = {}
        self.best_policies = {}
    ...
    def move_agent(self, index, action, atype):
        ...
        if action == 0: # do nothing
            ...
            hit_enemy = False
            if atype == "green":
                if space_val >= self.starts["purple"] and space_val < self.food_id:
                    hit_enemy = True
            else:
                if space_val >= self.starts["green"] and space_val < self.starts["purple"]:
                    hit_enemy = True
            if hit_enemy == True:
                enemy = "green"
                if atype == "green":
    ...
    def get_tile_val(self, tile, atype):
        ...
        if tile == 1:
            return 1
        elif tile >= self.starts["green"] and tile < self.starts["purple"]:
            return 2
        elif tile >= self.starts["purple"] and tile < self.food_id:
            return 3
    ...
    def get_nearest_enemy_directions(self, xpos, ypos, atype, num):
        ...
        positions = []
        if atype == "green":
            positions = np.argwhere((sp>=self.starts["purple"]) & (sp<self.food_id))
        else:
            positions = np.argwhere((sp>=self.starts["green"]) & (sp<self.starts["purple"]))
    ...
    def get_adjacent_friend_indices(self, index, atype):
        ...
        for i in offsets:
            ...
            if atype == "green" and tile == 2:
                ai = self.get_agent_at_position(xp, yp, atype)
                indices.append(ai)
                count += 1
            if atype == "purple" and tile == 3:
                ai = self.get_agent_at_position(xp, yp, atype)
                indices.append(ai)
                count += 1
        return indices
    ...
    def make_small_state(self, index, atype):
        ...
        directions = self.get_nearest_friend_directions(xgrid, ygrid, atype, 4)
        tiles = self.add_directions_to_state(tiles, directions)
    ...
    def get_printable(self, item):
        ...
        elif item >= self.starts["green"] and item < self.starts["purple"]:
            return "\x1b[1;32;40m" + "x" + "\x1b[0m"
        elif item >= self.starts["purple"] and item < self.food_id:
...
# Train the game
def msg(gs):
    colors = {"green":"32;40", "purple":"35;40"}
